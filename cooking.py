...
@bot.message_handler(func=lambda message: message.text == '🍽️ Добавить новое блюдо')
def get_user_data_for_dish(message):
    ...
    while True:
        ...
        ingridient_name = message.text

        bot.send_message(
            message.chat.id,
            'Введите стоимость ингридиента'
        )
        ingridient_price = message.text

        ingridients.append(
            [
                {
                    'name': ingridient_name,
                    'price': ingridient_price,
                }
            ]
        )
        ...
        bot.send_message(
            message.chat.id,
            'Добавить еще ингридиент?\nВведите Да или Нет'
        )

        if message.text != 'Да':
            add_dish(dish_name, ingridients)
            break


def add_dish(dish_name, ingridients):
    cooking_menu.append(
        {
            'id': str(uuid.uuid4()),
            'photo': None,
