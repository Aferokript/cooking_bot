...
def set_dish_name(message):
    ...
    bot.send_message(
        message.chat.id,
        text='Введите иструкцию по приготовленнию'
    )
    bot.register_next_step_handler(message, set_instruction)


def set_instruction(message):
    recipe = message.text
    temporary_dish_to_add['recipe'] = recipe
    bot.send_message(
        message.chat.id,
...
def add_dish(message):
    ...
    cooking_menu.append(
        {   
            ...
            'price': total_price,
            'instructions': f'{temporary_dish_to_add['recipe']}'
        }
    )
    del_temporary_data()
