...
users_main_menu.row('📋 Посмотреть избранные блюда', '❌ Удалить из избранного')
users_main_menu.row('💎 Купить бесконечные просмотры')

managers_id = {} # мужики не путайтесь, это такое множество
...
def show_dishes_logic(message):
    ...
    if user_free_dishes[user_id] > 0:
        bot.send_message(user_id, get_formatted_dishes([random_dish]), reply_markup=users_main_menu, parse_mode='Markdown')
...
def show_favorite_dishes(message):
    ...
    for dish in users_favorite_dishes[user_id]:
        bot.send_message(user_id, get_formatted_dishes([dish]), reply_markup=users_main_menu)
